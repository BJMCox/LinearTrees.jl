...
@testset "quantile and MAD gradients and IRLS weights" begin
    # the `r == 0` weight is a regression: gh reports an exact-zero gradient
    # there, which used to floor that row's weight to HMIN and bias the Newton
    # step away from a boundary quantile. l1weight gives the row its correct
    # one-sided weight (τ) instead.
...
@testset "deviance differences match Distributions.jl logpdf" begin
    ...
    for (loss, D, y) in ((Poisson(), f -> Distributions.Poisson(exp(f)), [0.0, 1.0, 3.0, 5.0]),
                         (Gamma(), f -> Distributions.Gamma(1.0, exp(f)), [0.5, 2.0, 1.3, 3.7]),
                         (NegBin(2.0), f -> Distributions.NegativeBinomial(2.0, 2.0 / (exp(f) + 2.0)), [0.0, 1.0, 4.0, 2.0]),
                         (Logistic(), f -> Distributions.Bernoulli(1 / (1 + exp(-f))), [0.0, 1.0, 1.0, 0.0]),
                         (MSE(), f -> Distributions.Normal(f, 1.0), [0.5, -1.2, 3.3, 0.0]))
        lhs, rhs = devdiff(loss, D, y, f1, f2, w)
    ...
    Dcat(f) = Distributions.Categorical(collect(LinearTrees.probs(loss, f)))
    lhs, rhs = devdiff(loss, Dcat, [1, 2, 3, 1], sf1, sf2, w)
    @test lhs ≈ rhs atol = 1e-10
end
...
@testset "wquantile_select! matches a sort-then-walk oracle" begin
    ...
    # longer change which row the walk returns on.
    ...
    ncases = 0
    for trial in 1:200
        n = trial <= 190 ? rand(rng, 1:64) : rand(rng, 65:3000)
